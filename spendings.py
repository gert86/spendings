...
def test(n_figs=10, backend="TKAgg", **kwargs):
    mpl.use(backend)
    plt.close("all")
...
#csv_file_path = config[config_key]['csv_file_path'].strip("'").strip('"')
...
for csv_file_path in csv_file_paths:
  ...
  for cat,data in categories_revenue.items():
    ...
    if le_sum != 0.0:
       Ys.append( [le_sum, 0.0, data["color"]] )  
  for cat,data in categories_spending.items():
    ...
    if le_sum != 0.0:
      Ys.append( [0.0, le_sum, data["color"]] )    
  # stacked bars
  fig, ax = plt.subplots()
  plt.title(f"Incomes and Spendings ({os.path.basename(csv_file_path)})")
  offset = np.array([0.0, 0.0])
  for y in Ys:
    ax.bar(x, y[0:2], bottom = offset, width=0.1, color=y[2])
    offset = np.add(offset, y[0:2])
  # legend
  legendHandles = []
  ...
  def on_add(sel):
      ...
      #print(f"{os.path.basename(csv_file_path)}: Type = {'Revenues' if round(x_index)==0 else 'Spendings'}, Height: {height}")
      category = list(categories.keys())[stack_index]
      text=f"{category}: {round(height,2)}€\n---------------------------------------------\n"  
      if category in categories.keys():
        regex_list = categories[category]["regex"]
        for r in regex_list:
